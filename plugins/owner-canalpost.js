  ...
  try {
    ...
    if (quoted && (mime.includes('image') || mime.includes('video'))) {
      ...
      if (mime.includes('image')) {
        messageContent = {
          image: buffer,
          caption: texto || `💙 *Hatsune Miku Bot - Publicación Oficial* 💙\n\n📅 *Fecha:* ${new Date().toLocaleString('es-MX')}\n🎵 *Publicado por:* @${m.sender.split('@')[0]}`,
          footer: '🌱 Hatsune Miku Bot - Canal Oficial'
        }
      } else if (mime.includes('video')) {
        messageContent = {
          video: buffer,
          caption: texto || `💙 *Hatsune Miku Bot - Video Oficial* 💙\n\n📅 *Fecha:* ${new Date().toLocaleString('es-MX')}\n🎵 *Publicado por:* @${m.sender.split('@')[0]}`,
          footer: '🌱 Hatsune Miku Bot - Canal Oficial'
        }
      }
    } 

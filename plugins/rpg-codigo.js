let handler = async (m, { conn, text }) => {
    ...
    if (isNaN(amount) || amount <= 0) {
        return conn.reply(m.chat, `${emoji} Por favor, ingrese una cantidad válida de ${moneda}.`, m, rcanal);
    }
    ...
    conn.reply(m.chat, `${emoji} Código generado: *${code}*\nEste código puede ser canjeado por ${amount} ${moneda} y puede ser utilizado por 50 personas.`, m, rcanal);
}

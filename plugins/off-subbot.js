...
return m.reply(`❌ Primero usa \`${usedPrefix}listsubbots\` para identificar los SubBots en este grupo.`)
}

const target = args[0]?.toLowerCase()
if (!target) {
return m.reply(`💡 *Uso del comando*\n\n\`${usedPrefix}offsubbot <número>\` - Apagar SubBot específico\n\`${usedPrefix}offsubbot todos\` - Apagar todos los SubBots\n\n📋 *SubBots disponibles:* ${global.lastSubBotsInGroup.length}`)
}
...
console.error('Error en offsubbot:', error)
return m.reply(`❌ Error: ${error.message}`)
}
}

handler.help = ['offsubbot']
handler.tags = ['serbot']
handler.command = ['offsubbot']

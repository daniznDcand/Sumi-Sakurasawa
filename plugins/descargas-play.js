...
async function downloadVideo(url) {
  try {
    const videoUrl = await mainApi.video(url);
    if (videoUrl) {
      console.log('✅ Descarga de video exitosa');
      return { url: videoUrl, title: 'Video', fuente: 'RapidAPI' };
    }
  } catch (error) {
    console.log(`❌ Error en descarga de video: ${error.message}`);
  }
...
async function ytdlAudio(url) {
  try {
    const audioUrl = await mainApi.audio(url);
    if (audioUrl) {
      console.log('✅ Descarga de audio exitosa');
      return { url: audioUrl, title: 'Audio', fuente: 'RapidAPI' };
    }
  } catch (error) {
    console.log(`❌ Error en descarga de audio: ${error.message}`);
  }

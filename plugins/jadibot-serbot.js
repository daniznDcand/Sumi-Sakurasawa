...
export async function mikuJadiBot(options) {
  ...
  try {
    ...
    if (m && conn) conn.reply(m.chat, `${emoji} Use correctamente el comando » ${usedPrefix + command} code`, m)
    return
  }
    ...
    async function connectionUpdate(update) {
      ...
      if (connection === 'close') {
        ...
        if (reason === 440) {
          ...
          try {
            if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(pathMikuJadiBot)}@s.whatsapp.net`, {text : '*HEMOS DETECTADO UNA NUEVA SESIÓN, BORRE LA NUEVA SESIÓN PARA CONTINUAR*\n\n> *SI HAY ALGÚN PROBLEMA VUELVA A CONECTARSE*' }, { quoted: m || null }) : ""
          } catch (error) {
      ...
      if (connection == `open`) {
        ...
        console.log(chalk.bold.cyanBright(`\n❒⸺⸺⸺⸺【• SUB-BOT •】⸺⸺⸺⸺❒\n│\n│ 🟢 ${userName} (+${path.basename(pathMikuJadiBot)}) conectado exitosamente. [Hatsune Miku 🌱💙]\n│\n❒⸺⸺⸺【• CONECTADO •】⸺⸺⸺❒`))
        sock.isInit = true

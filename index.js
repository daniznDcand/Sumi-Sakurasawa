...
const { proto } = (await import('@baileys-md/stable')).default
import pkg from 'google-libphonenumber'
const { PhoneNumberUtil } = pkg
const phoneUtil = PhoneNumberUtil.getInstance()
const { DisconnectReason, useMultiFileAuthState, MessageRetryMap, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, jidNormalizedUser, Browsers } = await import('@baileys-md/stable')
...
let { say } = cfonts
console.log(mikuPrimary(`\n${BRAND_EMOJI} Iniciando ${BRAND_NAME} Bot...`))
say('Hatsune\nMiku', {
  font: 'simple',
  align: 'left',
  gradient: ['cyan', 'green']
})
say('Made with (ㅎㅊDEPOOLㅊㅎ)', {
  font: 'console',
  align: 'center',
...
const colors = mikuPrimary
const qrOption = mikuPrimary
const textOption = mikuSecondary
...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${sessions}/creds.json`)) {
  do {
    ...
    if (!/^[1-2]$/.test(opcion)) {
      console.log(chalk.bold.redBright(`${brandTag} No se permiten números que no sean 1 o 2, tampoco letras o símbolos especiales.`))
    }
...
conn.isInit = false
conn.well = false
conn.logger.info(`${brandTag}  LISTO\n`)
if (!opts['test']) {
...
async function connectionUpdate(update) {
  ...
  if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
      ...
      console.log(chalk.bold.cyanBright(`\n${brandTag} Reconectando la conexión del Bot...`))
      await global.reloadHandler(true).catch(console.error)
    } else if (reason === DisconnectReason.connectionLost) {
      console.log(chalk.bold.cyanBright(`\n${brandTag} Conexión perdida con el servidor, reconectando el Bot...`))
      await global.reloadHandler(true).catch(console.error)
    } else if (reason === DisconnectReason.connectionReplaced) {
      console.log(chalk.bold.yellowBright(`\n${brandTag} La conexión del Bot ha sido reemplazada.`))
    } else if (reason === DisconnectReason.loggedOut) {
      console.log(chalk.bold.redBright(`\n${brandTag} Sin conexión, borra la sesión principal del Bot y conéctate nuevamente.`))
      await global.reloadHandler(true).catch(console.error)
    } else if (reason === DisconnectReason.restartRequired) {
      console.log(chalk.bold.cyanBright(`\n${brandTag} Conectando el Bot con el servidor...`))
      await global.reloadHandler(true).catch(console.error)
...
setInterval(() => {
  console.log(`${brandTag}  Reiniciando...`)
  process.exit(0)
}, 10800000)
...
function purgeSessionSB() {
  try {
    ...
    if (SBprekey.length === 0) {
      console.log(mikuSecondary.bold(`\n${brandTag} No hay archivos en ${jadi} para eliminar.`))
    } else {
      console.log(mikuPrimary.bold(`\n${brandTag} Archivos de la carpeta ${jadi} han sido eliminados correctamente.`))
    }
  } catch (err) {
    console.log(chalk.bold.red(`\n${brandTag} Error al eliminar archivos de la carpeta ${jadi}.\n` + err))
  }
}
      ...
      files.forEach(file => {
        if (file !== 'creds.json') {
          ...
          unlinkSync(filePath, err => {
            if (err) {
              console.log(chalk.bold.red(`\n${brandTag} El archivo ${file} no se logró borrar.\n` + err))
            } else {
              console.log(mikuSecondary.bold(`\n${brandTag} El archivo ${file} se ha borrado correctamente.`))
            } }) }
      }) }) }) }
...
setInterval(async () => {
  ...
  await clearTmp()
  console.log(mikuPrimary.bold(`\n${brandTag} Archivos temporales no necesarios han sido eliminados.`))
}, 1000 * 60 * 4)
setInterval(async () => {
  if (stopped === 'close' || !conn || !conn.user) return
  await purgeSession()
...
setInterval(async () => {
  if (stopped === 'close' || !conn || !conn.user) return
  await purgeOldFiles()
  console.log(mikuPrimary.bold(`\n${brandTag} Archivos no necesarios han sido eliminados del servidor.`))
}, 1000 * 60 * 10)

...
import { sendCompatibleMessage } from './compatible-messages.js'
...
sendGroupV4Invite: {
    ...
    // Nueva función mejorada para reemplazar rcanal
    sendMikuMessage: {
      async value(jid, text, quoted = null, options = {}) {
        ...
        // Intentar con newsletter si está disponible (WhatsApp Business)
        try {
          ...
          return conn.reply(jid, text, quoted, { contextInfo: newsletterContext, ...options });
        } catch (newsletterError) {
          // Si falla newsletter, usar versión normal (WhatsApp estándar)
          console.log('Newsletter no disponible, usando versión normal:', newsletterError.message);
          return conn.reply(jid, text, quoted, { contextInfo: enhancedContext, ...options });
        }
      }
    }, 
    /**
